  ...
  methods: {
    ...
    submitDataList() {
      if (this.isEdit) {
        ...
        this.form.solutiondetailCfgList[this.currentIdx].solutiondetailitemCfgList[this.listIdx] = JSON.parse(JSON.stringify(this.addModel))
      }
      this.open = false
      this.addModel = {
        title: '',
    ...
    handleAddService() {
      this.$refs['form'].validate((valid) => {
        if (valid) {
          if(this.level !== 2){
            this.$modal.msgError("解决方案分类只能选择第二级");
            return;
          }
