...
import { addProductservice, getAllPublishElement, listModuleMenu } from '@/api/dip/productservice'
  ...
  methods: {
    ...
    submitDataList() {
      ...
      if (this.isEdit) {
        ...
        this.form.productservicedetailCfgList[this.currentIdx].productservicedetailitemCfgList[this.listIdx] = JSON.parse(JSON.stringify(this.addModel))
      }
      this.open = false
      this.addModel = {
        title: '',
    ...
    // 删除数据项列表
    delDatalist(idx) {
      this.form.productservicedetailCfgList[this.currentIdx].productservicedetailitemCfgList.splice(idx, 1)
    },
    // 编辑数据项列表
    ...
    handleAddService() {
      ...
      this.$refs['form'].validate((valid) => {
        if (valid) {
          ...
          addProductservice(this.form).then(res => {
            if (res.code == 200) {
              this.loading.close();
