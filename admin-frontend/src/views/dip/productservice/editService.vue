<template>
  <div class="app-container">
    <el-form ref="form" :model="form" :rules="rules" label-width="140px">
      ...
      <el-form-item label="产品服务分类" prop="menuId" v-if="form.lanType!=null">
        <treeselect
          v-model="form.menuId"
          :options="menuOptions"
      ...
      <el-form-item label="产品服务描述" prop="serviceDes">
        <el-input
          v-model="form.serviceDes"
          placeholder="请输入产品服务描述"
          style="width:223.6px"
        />
      </el-form-item>
      ...
      <el-form-item
        ...
        label="模板"
        prop="productservicedetailCfgList"
        key="productservicedetailCfgList"
      >
        <div class="template" v-for="(item,idx) of form.productservicedetailCfgList" :key="idx">
          <div class="desc">
          ...
          <div class="content">
            ...
            <div class="bottom">
              <div class="dataList">
                <div>数据项列表</div>
                <div>
                  <el-button type="primary" plain icon="el-icon-plus" size="mini" @click="addDataList(item, idx)">
                    添加数据项
                  </el-button>
                </div>
              </div>
              <div class="table">
                <el-table
                  :data="item.productservicedetailitemCfgList"
                    ...
                    <template slot-scope="scope">
                      <div class="operation">
                        ...
                        <el-button
                          type="danger"
                          size="mini"
                          @click="delDatalist(scope.row.id,scope.$index)">删除</el-button>
                      </div>
                    </template>
                  </el-table-column>
          ...
          <div class="sortList">
            ...
            <span v-if="idx < form.productservicedetailCfgList.length - 1" @click="templateDown(idx)"><i class="el-icon-arrow-down"></i></span>
          </div>
        </div>
      </el-form-item>
    ...
    <el-dialog title="添加数据项" :visible.sync="open" width="700px" destroy-on-close append-to-body>
      <el-form :model="addModel" :rules="rules" label-width="140px">
        ...
        <el-form-item label="描述项">
          ...
          <div class="addTable">
            <el-table
              :data="addModel.productservicedetailitemdescCfgList"
              :header-cell-style="headStyle"
              :cell-style="headStyle"
              style="width: 100%"
            >
...
export default {
  ...
  data() {
    ...
    return {
      ...
      addModel: {
        ...
        url: '',
        desc: '',
        productservicedetailitemdescCfgList: []
      },
      // 表单校验
      rules: {
        lanType: [
        ...
        serviceDes: [
          { required: true, message: '产品服务描述不能为空', trigger: 'blur' },
          { max: 255, message: '请输255个字节以内的服务描述', trigger: 'blur' }
        ],
        tag: [
        ...
        //   { required: true, message: '请选择关联市场元素', trigger: 'change' }
        // ],
      }
    }
  },
  ...
  methods: {
    ...
    getProductList() {
      ...
      getProductservice(id).then(res => {
        if (res.code === 200) {
          let dataList = res.data.productservicedetailCfgList
          if (dataList) {
            dataList.map((v, i) => {
              this.templateIdx = i
            })
    ...
    getTreeselect() {
      listModulemenu({ 'type': 0, 'lanType': this.form.lanType }).then(response => {
        this.menuOptions = []
    ...
    // 添加模板
    addTemplate() {
      this.form.productservicedetailCfgList.push(
        {
          templateType: '1',
          type: '',
          productservicedetailitemCfgList: []
        }
      )
    },
    // 删除模板
    delTemplate(idx) {
      this.form.productservicedetailCfgList.splice(idx, 1)
    },
    // 模板上移
    templateUp(idx) {
      console.log('templateUp', idx)
      let arr = this.form.productservicedetailCfgList
    ...
    // 删除数据项列表
    delDatalist(id, idx) {
      let parent = this.form.productservicedetailCfgList
      for (let i = 0; i < parent.length; i++) {
        parent[i].productservicedetailitemCfgList.splice(idx, 1)
      }
    },
    // 添加描述项
    handleAddDescribe() {
      this.addModel.productservicedetailitemdescCfgList.push(
        {
          type: '',
          content: ''
        }
    ...
    delDescribedelDescribe(idx) {
      this.addModel.productservicedetailitemdescCfgList.splice(idx, 1)
    },
    // 取消弹出框
    cancel() {
      this.open = false
    ...
    submitDataList() {
      if (this.isEdit) {
        this.form.productservicedetailCfgList[this.currentIdx].productservicedetailitemCfgList.push(JSON.parse(JSON.stringify(this.addModel)))
      }
      this.open = false
      this.addModel = {
        title: '',
    ...
    changeVal(val) {
      ...
      if (this.form.isConfig !== 0 && this.form.showType == 1) {
        this.form.productservicedetailCfgList = []
        this.addTemplate()
      }
    },
    //  提交表单
      ...
      this.$refs['form'].validate((valid) => {
        if (valid) {
          ...
          if (this.form.isConfig !== 1 || this.form.showType == 2) {
            delete this.form.productservicedetailCfgList
          }
          ...
          updateProductservice(this.form).then(res => {
            if (res.code == 200) {
              this.loading.close()
